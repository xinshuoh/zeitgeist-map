...
function App() {
  ...
  const displayCountryData = async (e: LeafletMouseEvent) => {
    ...
    if (isCompareMode) {
      const countrySimilarities: CountrySimilarityData[] = (await fetchCountryCompareData(countryCode)) as CountrySimilarityData[];
      // Apply red coloring based on similarity data
      countrySimilarities.forEach(similarity => {
        const countryLayer = (layer as any)._map._layers[similarity.country_code];
        if (countryLayer) {
  ...
  const doHeatMap = async () => {
    setCompareMode(true);
    alert("Click on a country to compare it with the rest of the world.");
  };

  const resetMapStyle = () => {
    setCompareMode(false);
    // Reset all layers to their original style
    (worldGeoJSON as any).features.forEach((feature: Feature<Geometry, GeoJsonProperties>) => {
      const layerId = feature.id;
      if (layerId !== undefined) {
        const layer = (feature as any)._map._layers[layerId];
        if (layer) {
  ...
  return (
    <>
      ...
      <div id="map-container" className="flex">
          ...
          >
            {selectedCountry && (
              <Popup>
                ...
                <ul>
                  {selectedCountry.songlist.slice(0, 5).map((song: any) => <li key={song.song_name}>{song.song_name}</li>)}
                </ul>
        ...
        {isCompareMode && (
          <button onClick={resetMapStyle} style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 1000 }}>
            Exit Compare Mode
          </button>
        )}
      </div>
